"""Walkable-map utilities for IntentionNav eval.
...
# Override via INTENTIONNAV_METAROOT env var.
...
class WalkableMap:
    ...
    def shortest_path_2d(
        self,
        p0: tuple[float, float],
        p1: tuple[float, float],
        snap_radius_m: float = 2.0,
    ) -> list[tuple[float, float]] | None:
        """Return an A* path on the walkable grid in world coordinates.

        Object centers are commonly on occupied cells, so each endpoint is
        snapped to its nearest walkable cell within ``snap_radius_m``. The
        returned path includes both snapped endpoints. ``None`` means no valid
        endpoint or no connected path exists.
        """
        if len(self.x_coords) < 2 or len(self.y_coords) < 2:
            return None

        def snap_cell(point: tuple[float, float]) -> tuple[int, int] | None:
            if self.is_walkable(*point):
                return self.world_to_cell(*point)
            nearby = self.nearby_walkable(*point, radius_m=snap_radius_m)
            return self.world_to_cell(*nearby) if nearby is not None else None
        ...
        while open_heap:
            ...
            for dyi, dxi, move_cost in neighbors:
                neighbor = (current[0] + dyi, current[1] + dxi)
                if not (0 <= neighbor[0] < h and 0 <= neighbor[1] < w):
                    continue
                if not self.grid[neighbor] or neighbor in closed:
                    continue
                new_g = g + move_cost
