...
# Frontier utility terms.  Candidate selection is restricted to the actual
# explored/unexplored boundary; this prevents a flat room prior from choosing
# an arbitrary interior cell.  The local unknown-area term approximates the
# geometric information gain used by classical frontier planners, while the
# travel term avoids repeatedly selecting a very distant tie.
FRONTIER_INFO_WEIGHT = float(os.environ.get("VM_FRONTIER_INFO_WEIGHT", "0.25"))
FRONTIER_TRAVEL_WEIGHT = float(os.environ.get("VM_FRONTIER_TRAVEL_WEIGHT", "0.12"))
...
# Cells in an unvisited likely room get a soft boost. The prior must remain
# comparable to, rather than dominate, frontier information gain: generic
# object-room priors match only part of the diverse Kujiale layouts.
ROOM_PRIOR_UNVISITED_BOOST = float(
    os.environ.get("VM_ROOM_PRIOR_UNVISITED_BOOST", "1.5")
)
...
class ValueMap:
    ...
    def __init__(self, walkable_map, plan: dict):
        ...
        # UIAP-OGN-style probabilistic target relevance belief.  The
        # uninformed N(0.5, 0.5) prior treats unseen directions as plausible
        # and uncertain rather than assigning them zero semantic value.
        self._prob_visual_mean = np.full(
            (self.h, self.w), 0.5, dtype=np.float32
        )
        self._prob_visual_variance = np.full(
            (self.h, self.w), 0.5, dtype=np.float32
        )
        self._prob_visual_observed = np.zeros(
            (self.h, self.w), dtype=np.bool_
        )
        self.last_frontier = np.zeros((self.h, self.w), dtype=np.bool_)
        self.last_information_gain = np.zeros((self.h, self.w), dtype=np.float32)
    ...
    def build(self, target_memory: list[dict],
              ...
              room_prior_scale: float = 1.0) -> np.ndarray:
        ...
        # ``room_prior_scale=0`` is used by the geometry-only frontier
        # treatment.  Keeping this as an explicit call-site option makes the
        # ablation independent of category-to-room heuristics while preserving
        # the historical behavior for every existing caller.
        v += ROOM_WEIGHT * float(room_prior_scale) * room_layer
        ...
        if ec is not None and isinstance(ec, np.ndarray) \
            ...
            # Local unexplored fraction is a cheap area-potential proxy.  It
            # makes wide doorways/open rooms preferable to one-cell slivers.
            try:
                from scipy.ndimage import uniform_filter
                unexplored = unvisited_walkable.astype(np.float32)
                self.last_information_gain = uniform_filter(
                    unexplored, size=9, mode="constant"
                )
            except Exception:
                self.last_information_gain = unvisited_walkable.astype(np.float32)
            if frontier.any():
                ...
                self.last_frontier = self.walkable.copy()
                self.last_information_gain = self.walkable.astype(np.float32)
        else:
    ...
    def observe_visual_hint(self, direction_hint: dict | None) -> dict:
        """Fuse one non-terminal visual search hint into persistent memory.

        This updates only the VLFM-style cone state.  It does not generate a
        navigation goal, target cluster, or STOP evidence.
        """
        if direction_hint is None:
            return {"updated": False, "max_value": 0.0, "max_confidence": 0.0}
        before = self._cone_value.copy()
        scratch = np.zeros((self.h, self.w), dtype=np.float32)
        self._paint_cone(scratch, direction_hint)
        return {
            "updated": bool(np.any(np.abs(self._cone_value - before) > 1e-8)),
            "max_value": float(np.max(self._cone_value)),
            "max_confidence": float(np.max(self._cone_conf)),
        }

    def visual_memory_value_at(self, wx: float, wy: float) -> float:
        """Return the persistent target-visual value at a walkable waypoint."""
        try:
            yi, xi = self.wm.world_to_cell(float(wx), float(wy))
        except Exception:
            return 0.0
        if not (0 <= yi < self.h and 0 <= xi < self.w):
            return 0.0
        return float(max(0.0, self._cone_value[yi, xi]))

    def observe_probabilistic_visual_hint(
        self, direction_hint: dict | None
    ) -> dict:
        """Bayesian-update semantic mean/variance in the observed focal cone.

        The update follows UIAP-OGN's conjugate Gaussian semantic map. Prompt
        ensemble variance is augmented by viewpoint uncertainty ``1-cos²``;
        RGB-D visibility prevents updates behind observed surfaces.
        """
        if direction_hint is None:
            return {"updated": False, "updated_cells": 0}
        try:
            agent_xy = direction_hint["agent_xy"]
            yaw = float(direction_hint["yaw"])
            observation_mean = min(
                1.0, max(0.0, float(direction_hint["relevance_mean"]))
            )
            prompt_variance = max(
                1e-6, float(direction_hint["relevance_variance"])
            )
        except (KeyError, TypeError, ValueError):
            return {"updated": False, "updated_cells": 0}
        cells_r = int(math.ceil(CONE_RANGE_M / self.cell_m))
        cy_idx, cx_idx = self.wm.world_to_cell(agent_xy[0], agent_xy[1])
        y0, y1 = max(0, cy_idx - cells_r), min(self.h, cy_idx + cells_r + 1)
        x0, x1 = max(0, cx_idx - cells_r), min(self.w, cx_idx + cells_r + 1)
        if y1 <= y0 or x1 <= x0:
            return {"updated": False, "updated_cells": 0}
        ...
        for local_y, global_y in enumerate(range(y0, y1)):
            for local_x, global_x in enumerate(range(x0, x1)):
                wx_grid[local_y, local_x], wy_grid[local_y, local_x] = (
                    self.wm.cell_to_world(global_y, global_x)
                )
        dx_arr = wx_grid - float(agent_xy[0])
        dy_arr = wy_grid - float(agent_xy[1])
        distance = np.hypot(dx_arr, dy_arr)
        bearing = np.arctan2(dy_arr, dx_arr)
        angle = np.arctan2(np.sin(bearing - yaw), np.cos(bearing - yaw))
        hfov = float(direction_hint.get("hfov_rad", CONE_HFOV_RAD))
        in_fov = (np.abs(angle) <= hfov / 2.0 + 1e-3) & (
            distance <= CONE_RANGE_M
        )
        visible = in_fov
        ...
        if depth is not None:
            try:
                ...
                pixel = np.clip(
                    ((0.5 - angle / hfov) * (width - 1)).astype(np.int32),
                    0,
                    width - 1,
                )
                depth_at_pixel = depth[height // 2, pixel]
                visible = in_fov & np.isfinite(depth_at_pixel) & (
                    distance <= depth_at_pixel + 0.3
                )
            except Exception:
                visible = in_fov
        angle_normalized = np.clip(
            np.abs(angle) / max(1e-6, hfov / 2.0), 0.0, 1.0
        )
        viewpoint_confidence = np.cos(
            angle_normalized * (math.pi / 2.0)
        ) ** 2
        update_mask = visible & self.walkable[y0:y1, x0:x1] & (
            viewpoint_confidence > 1e-4
        )
        if not update_mask.any():
            return {"updated": False, "updated_cells": 0}
        prior_mean = self._prob_visual_mean[y0:y1, x0:x1]
        prior_variance = self._prob_visual_variance[y0:y1, x0:x1]
        observation_variance = prompt_variance + (1.0 - viewpoint_confidence)
        denominator = prior_variance + observation_variance
        posterior_mean = (
            prior_variance * observation_mean
            + observation_variance * prior_mean
        ) / np.maximum(denominator, 1e-8)
        posterior_variance = (
            prior_variance * observation_variance
        ) / np.maximum(denominator, 1e-8)
        prior_mean[update_mask] = posterior_mean[update_mask]
        prior_variance[update_mask] = posterior_variance[update_mask]
        self._prob_visual_observed[y0:y1, x0:x1][update_mask] = True
        return {
            "updated": True,
            "updated_cells": int(update_mask.sum()),
            "observation_mean": float(observation_mean),
            "prompt_variance": float(prompt_variance),
            "map_mean_min": float(prior_mean[update_mask].min()),
            "map_mean_max": float(prior_mean[update_mask].max()),
            "map_variance_min": float(prior_variance[update_mask].min()),
            "map_variance_max": float(prior_variance[update_mask].max()),
        }

    def probabilistic_visual_belief_at(
        self, wx: float, wy: float
    ) -> tuple[float, float]:
        """Return posterior semantic mean and variance at a waypoint."""
        try:
            yi, xi = self.wm.world_to_cell(float(wx), float(wy))
        except Exception:
            return 0.5, 0.5
        if not (0 <= yi < self.h and 0 <= xi < self.w):
            return 0.5, 0.5
        return (
            float(self._prob_visual_mean[yi, xi]),
            float(self._prob_visual_variance[yi, xi]),
        )
    ...
    def next_waypoint(self, current_pose_xy: tuple[float, float],
        ...
        v = self.build(target_memory, agent_path,
                       ...
                       room_prior_scale=room_prior_scale)
        ...
        v[~reachable] = -1e9
        # Frontier utility selection.  Room/semantic values are evaluated at
        # reachable frontiers, rather than over every cell in an unvisited
        # room.  This follows the common ObjectNav separation between
        # candidate generation and semantic scoring.
        candidate_mask = reachable & self.last_frontier
        if not candidate_mask.any():
            candidate_mask = reachable & self.walkable
        yy, xx = np.indices((self.h, self.w))
        cell_distance = np.hypot(yy - cur_yi, xx - cur_xi) * self.cell_m
        scene_diag_m = max(self.cell_m, math.hypot(self.h, self.w) * self.cell_m)
        utility = (
            v
            + FRONTIER_INFO_WEIGHT * self.last_information_gain
            - FRONTIER_TRAVEL_WEIGHT * (cell_distance / scene_diag_m)
        )
        utility[~candidate_mask] = -1e9
        flat_idx = int(np.argmax(utility))
        ...
        meta = {"target_cell": list(target_cell),
                ...
                "frontier_utility": float(utility[target_cell]),
                "candidate_mode": "frontier" if self.last_frontier[target_cell]
                                  else "reachable_fallback",
                "path_len_cells": None,
                "chosen_path_idx": None,
                "room_prior_scale": float(room_prior_scale),
    ...
    def _paint_cone(self, v: np.ndarray, direction_hint: dict) -> None:
        ...
        # Hosted-VLM hints historically provide only an integer score.  The
        # target-visual-memory treatment additionally supplies the underlying
        # rank percentile so fusion does not quantize away useful ordering.
        relevance_value = direction_hint.get("relevance_value")
        if relevance_value is None:
            relevance_value = rel / 10.0
        relevance_value = min(1.0, max(0.0, float(relevance_value)))
        peak = relevance_value * CONE_WEIGHT
