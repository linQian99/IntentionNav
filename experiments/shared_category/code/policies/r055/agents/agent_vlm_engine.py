...
from navigation_motion import resolve_navigation_move
...
def _backproject_pixel_to_world(px_norm: float, py_norm: float,
    ...
    """Project axial image-plane depth through a yaw-only pinhole camera.

    Isaac's distance_to_image_plane is forward depth, not ray length.
    R055's 90-degree camera has fx=W/2 and cx=W/2. Retain its pixel
    rounding and depth sampling; change only the camera geometry.
    """
    ...
    focal_x = W_d / (2.0 * math.tan(hfov_rad / 2.0))
    forward = d
    left = -(px - W_d / 2.0) * d / focal_x
    yaw = pose["yaw"]
    return (pose["position"][0] + forward * math.cos(yaw) - left * math.sin(yaw),
            pose["position"][1] + forward * math.sin(yaw) + left * math.cos(yaw))
...
def run_episode_engine(env, wm, item, episode_meta, style, model_key,
                       ...
                       shared_query_inputs: Path | None = None,
                       shared_input_mode: str = "explicit"):
    """Engine-driven episode loop. See module docstring."""
    sel_id = item["selection_id"]
    epi_dir = out_path.parent
    from r055_observation_archive import ObservationArchive
    observation_archive = ObservationArchive(epi_dir, step_cap)
    observation_archive_metadata = None
    if (not local_perception_only or not objectnav or not VM_DINO_LABEL_STRICT
            or not INAV_CLIP_VERIFY or VM_NO_ADMIT or INAV_COCO_SPECIALIST
            or INAV_BUDGETED_VIEW_SCAN or INAV_DUAL_DETECTOR_FUSION):
        raise ValueError("Archive supports only the declared R055 DINO/CLIP diagnostic configuration")
    execution_seed = _seed_episode_execution(sel_id, style)

    shared_input_provenance = None
    if shared_query_inputs is not None and (not objectnav or not local_perception_only
            or robustness_mode != "none" or scene_candidates):
        raise ValueError("Shared inputs require plain local ObjectNav")
    if shared_query_inputs is None and shared_input_mode != "explicit":
        raise ValueError("Implicit input requires frozen shared query records")
    ...
    if target_absent:
        ...
        if shared_query_inputs is not None:
            tgt_cat, shared_input_provenance = resolve_category(
                shared_query_inputs, item=item, style=style, input_mode=shared_input_mode)
        else:
    ...
    record_model = (
        ...
        else robustness_output_model_name(model_key, robustness_mode)
    )
    ...
    for step in range(1, step_cap + 1):
        ...
        archive_dets, archive_strict, archive_verifications = [], [], []
        archive_selected, archive_error = None, None
        try:
            phrases = list(target_queries)
            if not INAV_TARGET_ONLY_DINO:
                for c in candidates:
                ...
                if (not VM_NO_ADMIT) and dets:
                    ...
                    if target_guess and VM_DINO_LABEL_STRICT:
                        ...
                        if strict_dets and detector_source in {
                            ...
                            archive_verifications = clip_results
            ...
            if local_perception_only:
                observation_archive.capture(
                    step=step, pose={"position": list(pose["position"]), "yaw": pose["yaw"]},
                    rgb=rgb, depth=depth, detections=archive_dets,
                    strict_detections=archive_strict, verifications=archive_verifications,
                    selected=archive_selected, evidence=current_dino_evidence, error=archive_error)
                raise
            if (INAV_DUAL_DETECTOR_FUSION
                    or (
                        (
        ...
        # Every proposal, including value-map fallback and tie-breaking creep,
        # must pass the same final MOVE contract before any arrival bookkeeping.
        if approach_result is None:
            (nx, ny), nav_meta = vmap.next_waypoint(
                current_xy, target_memory, agent_path,
        ...
        if approach_result is not None:
            ...
            (nx, ny), motion_meta = resolve_navigation_move(
                wm, current_xy, (nx, ny), maximum=1.7,
            )
            nav_meta = {**nav_meta, "motion_validation": motion_meta}
            if motion_meta["adjusted"]:
                # A route prefix is not arrival at the requested verification
                # camera. Do not record a verification or commitment arrival.
                navigation_cluster = None
                navigation_cluster_kind = None
                nav_meta["arrived_standoff"] = False
                nav_meta["novel_view_used"] = False
    ...
    observation_archive_metadata = observation_archive.finish(traj)
    # ---- Episode end ----
    ...
    record = {
        ...
        "plan": {**plan,
                 "plan_model": ("deterministic_explicit_target"
                                if local_perception_only
                                else MODEL_CATALOG[model_key]["model"]),
        ...
        "model_meta": {
            "provider": ("local" if local_perception_only
                         else MODEL_CATALOG[model_key]["provider"]),
    ...
    if shared_input_provenance is not None:
        record["navigation_input"] = shared_input_provenance
        record["evaluation_protocol"]["goal_input"] = "shared_" + shared_input_mode
        record["evaluation_protocol"]["metric_scope"] = ("navigation_only"
            if shared_input_mode == "explicit" else "category_bottleneck_full_chain")
        record["plan"]["plan_model"] = "provided_shared_category"
    if robustness_meta:
        record["robustness"] = robustness_meta
    record["observation_archive"] = observation_archive_metadata
...
def main():
    ...
    ap.add_argument("--shared-query-inputs", type=Path)
    ap.add_argument("--shared-input-mode", choices=["explicit", "implicit_category"], default="explicit")
    ap.add_argument("--check-navigation-inputs", action="store_true")
    ap.add_argument(
        "--local-perception-only",
        action="store_true",
        help="No-API explicit ObjectNav calibration using the same IntentionNav "
             "frontier/evidence/approach policy and local GroundingDINO.",
    )
    ...
    if args.local_perception_only and not args.objectnav:
        ap.error("--local-perception-only requires --objectnav")
    ...
    if args.shared_query_inputs is not None:
        if not args.objectnav or not args.local_perception_only or args.robustness != "none":
            ap.error("Shared input requires plain --objectnav --local-perception-only")
        query_items = load_items()
        if args.only:
            wanted = {s.strip() for s in args.only.split(",") if s.strip()}
            query_items = [item for item in query_items if item["selection_id"] in wanted]
            if {item["selection_id"] for item in query_items} != wanted:
                ap.error("Requested shared input identities are missing")
        if args.limit:
            query_items = query_items[:args.limit]
        checked = 0
        for item in query_items:
            for style in (STYLES if args.style == "all" else (args.style,)):
                resolve_category(args.shared_query_inputs, item=item, style=style,
                    input_mode=args.shared_input_mode)
                checked += 1
        if not checked:
            ap.error("No shared navigation inputs requested")
        if args.check_navigation_inputs:
            print(json.dumps({"passed": True, "queries": checked,
                "input_mode": args.shared_input_mode, "gpu_initialized": False, "output_created": False}))
            return
    elif args.shared_input_mode != "explicit" or args.check_navigation_inputs:
        ap.error("Shared implicit/check mode requires --shared-query-inputs")
    ...
    def _queue_run():
        ...
        while True:
            ...
            tier_name = ("explicit_objectnav" if args.objectnav else
                         ...
                         "vlm_engine")
            record_model = (
                "intentionnav_local" if args.local_perception_only
                else robustness_output_model_name(item_model, args.robustness)
            )
            ...
            for style in ep_styles:
                ...
                try:
                    run_episode_engine(
                        ...
                        shared_query_inputs=args.shared_query_inputs,
                        shared_input_mode=args.shared_input_mode,
                    )
                    n_styles_done += 1
                except Exception as e:
                    import traceback
    ...
    try:
        ...
        for idx, item in enumerate(items):
            ...
            record_model = (
                "intentionnav_local" if args.local_perception_only
                else robustness_output_model_name(args.model, args.robustness)
            )
            path_model = record_model if tier_name == "vlm_engine" else f"{tier_name}_{record_model}"
            for style in styles:
