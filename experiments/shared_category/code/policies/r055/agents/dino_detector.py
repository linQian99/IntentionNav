...
MODEL_REVISION = os.environ.get(
    "DINO_MODEL_REVISION", "a2bb814dd30d776dcf7e30523b00659f4f141c71"
)


def _ensure_loaded():
    """Lazy-load model on first call. Cheap on subsequent calls."""
    global _proc, _model, _device, _model_id
    ...
    with _lock:
        ...
        _proc = AutoProcessor.from_pretrained(
            _model_id, revision=MODEL_REVISION
        )
...
def configured_model_id() -> str:
    """Return the configured detector checkpoint without loading weights."""
...
def detect(rgb_np, query_phrases: List[str],
    ...
    Returns:
      ...
      Empty list if no detections; diagnostic inference errors propagate.
    """
    ...
    try:
        ...
        raise RuntimeError("Required DINO inference failed") from e
