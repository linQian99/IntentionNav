...
from common import (
    ...
    normalize_robustness_mode, robustness_output_model_name,
)  # noqa: E402
...
def build_episode_system(intent: str, plan: dict | None,
                         robustness_note: str | None = None) -> str:
    ...
    if robustness_note:
        ctx_lines.append(f"Robustness note: {robustness_note}")
    return sys_text + "\n\n" + "\n".join(ctx_lines)
...
def run_episode(env, wm: WalkableMap, item: dict, episode_meta: dict,
                ...
                out_path: Path, objectnav: bool = False,
                robustness_mode: str = "none",
                robustness_seed: int = 0) -> dict:
    ...
    if robustness_meta and robustness_meta.get("mode") == "target_absent":
        ...
        plan = {
            ...
            "likely_rooms": [],
            "strategy": (
                f"Search the scene for a {tgt_cat}; do not STOP unless the "
        ...
        tgt_cat = (
            ...
            or episode_meta.get("target_category")
            or ""
        ).strip()
        if not tgt_cat:
            raise ValueError(
        ...
        plan = {
            "target_guess": tgt_cat,
            "candidate_objects": [tgt_cat],
            # ObjectNav exposes the category, not the ground-truth room.
            "likely_rooms": [],
    ...
    episode_system = build_episode_system(intent, plan, robustness_note)
    ...
    for step in range(1, step_cap + 1):
        ...
        if action == "STOP":
            ...
            # STOP is terminal and cannot translate the agent. Older runs
            # included a hidden one-metre creep after the decision, which
            # inflated terminal localization and was not a standard action.
            pose = env.get_pose()
    ...
    record = {
        ...
        # Keep the label immutable; inferred targets belong in prediction/plan.
...
def main():
    ...
    if args.robustness == "target_absent" and args.objectnav:
        ap.error("--robustness target_absent already constructs an explicit "
                 "ObjectNav negative task; do not combine it with --objectnav")
    ...
    # unknown args as Kit options. On some hardware/driver combos (e.g. 4090 +
    # CUDA 13 driver), a single Kit start hangs/aborts silently when it sees
    # our --model / --only / etc. Local 3090 + CUDA 12 happens to tolerate
    # them. Argparse already consumed the values into `args`, so clearing
    # sys.argv is safe.
    import sys as _sys
    ...
    def _queue_run():
        ...
        while True:
            ...
            path_model = robustness_output_model_name(item_model, args.robustness)
            out_path = episode_path(item["scene_id"], "vlm", path_model, style, sel_id)
    ...
    try:
        ...
        for idx, item in enumerate(items):
            ...
            for style in styles:
                path_model = robustness_output_model_name(args.model, args.robustness)
                out_path = episode_path(item["scene_id"], "vlm", path_model, style, sel_id)
                if out_path.exists() and not args.force:
                    continue
                try:
