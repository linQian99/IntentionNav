"""LLM-as-judge framework via 2077ai pp-api proxy (default model: gpt-5.5).
...
ENV:
  ...
  PP_API_KEY        required when calling _call_judge
  JUDGE_MODEL       optional override; default gpt-5.5
  PP_API_PROXY      optional per-call proxy (e.g. clash mihomo)
"""
...
REPO = _THIS.parents[2]  # eval/judge/judge.py → IntentEQA/
EVAL_DIR = REPO / "eval"
...
def _call_judge(user_prompt: str, img_bytes: bytes | None,
    ...
    """Single judge call via PP-API OpenAI-compatible endpoint."""
    ...
    _proxy_url = os.environ.get("PP_API_PROXY")
    if _proxy_url:
