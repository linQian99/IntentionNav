"""Occlusion-aware visibility checks for active IntentEQA trajectories.

This module intentionally duplicates the pure geometry used by
`intenteqa/capture_surfaces.py` instead of importing it: that capture script
...
DATASET_ROOT = REPO / "results/dataset"
DATASET_JSONL = DATASET_ROOT / "selected_500_intents.jsonl"
DEFAULT_SCENE_SUMMARY = Path(
    "/path/to/workspace/datasets/vlntube/SceneSummary/kujiale_scene_summary"
)
...
@lru_cache(maxsize=1)
def _items_by_selection_id() -> dict[str, dict]:
    out = {}
...
def _scene_manifest(scene_id: str) -> list[dict]:
...
@lru_cache(maxsize=256)
def _scene_objects(scene_id: str) -> list[dict]:
    root = Path(os.environ.get("INTENTEQA_SCENE_SUMMARY", str(DEFAULT_SCENE_SUMMARY)))
...
def _target_entry(record: dict) -> dict | None:
    scene_id = record.get("scene_id")
    if not scene_id:
        return None
    manifest = _scene_manifest(scene_id)
    if not manifest:
        return None

    item = _items_by_selection_id().get(record.get("selection_id", ""), {})
...
def _combined_target_bbox(record: dict):
    entry = _target_entry(record)
    if not entry:
        return None, None
    bmin_out = None
    bmax_out = None
...
def trajectory_visibility(record: dict,
                          ...
                          max_occlusion_ratio: float = MAX_OCCLUSION_RATIO) -> dict:
    ...
    if not traj:
        return {}
    bbox_min, bbox_max = _combined_target_bbox(record)
    if not bbox_min or not bbox_max:
        return {}

    target_center = _target_center(bbox_min, bbox_max)
    scene_objects = _scene_objects(record.get("scene_id", ""))
    best = {
        "G_seen": 0,
